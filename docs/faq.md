...
## なぜ`MessageRepository::create`は`ActiveModel::insert`を使用せずにSQL文でStatementを作成しているのですか？

`ActiveModel::insert`は、SeaORM 1.1.20 では内部的に `exec_with_returning` を呼びます（モデルを返すため）。
PostgreSQL は RETURNING で参照する列に対して SELECT 権限を要求します。
今回、`message`テーブルのSELECT権限はlambdaロールに付与しておらず、`messages_latest`ビューのSELECT権限のみ付与しているため、INSERT文を直接書いています。
これは SeaORM の仕様が変わらない限り変更予定はありません。
